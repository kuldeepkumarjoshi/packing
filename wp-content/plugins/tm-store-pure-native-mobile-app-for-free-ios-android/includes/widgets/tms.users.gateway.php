...
function tms_process_login_new_users_gateway( $provider, $redirect_to, $hybridauth_user_profile )
{
	// HOOKABLE:
	do_action( "tms_process_login_new_users_gateway_start", $provider, $redirect_to, $hybridauth_user_profile );

	$assets_base_url = WORDPRESS_TM_STORE_PLUGIN_URL . 'assets/img/16x16/';

	// remove tms widget
	remove_action( 'register_form', 'tms_render_auth_widget_in_wp_register_form' );
	...
	$requested_user_email        = apply_filters( 'tms_new_users_gateway_alter_requested_email', $requested_user_email );
	$requested_user_login        = apply_filters( 'tms_new_users_gateway_alter_requested_login', $requested_user_login );
	...
	$linking_enabled = get_option( 'tms_settings_bouncer_accounts_linking_enabled' );
        $require_email   = get_option( 'tms_settings_bouncer_profile_completion_require_email' );
        $change_username = get_option( 'tms_settings_bouncer_profile_completion_change_username' );
        $extra_fields    = get_option( 'tms_settings_bouncer_profile_completion_hook_extra_fields' );

	if( isset( $_REQUEST["bouncer_account_linking"] ) )
	{
		if( $linking_enabled == 2 )
		{
			return tms_process_login_render_notice_page( _tms__( "Not tonight.", 'wordpress-tm-store' ) );
		}
		...
		else
		{
			...
			*/
			...
			if( $require_email == 1 )
			{
				...
				{
					$profile_completion_errors[] = _tms__( '<strong>ERROR</strong>: Please type your e-mail address.', 'wordpress-tm-store' );
				}

				if ( ! is_email( $requested_user_email ) )
				{
					$profile_completion_errors[] = _tms__( '<strong>ERROR</strong>: Please enter a valid email address.', 'wordpress-tm-store' );
				}

				if ( tms_wp_email_exists( $requested_user_email ) )
				{
					$profile_completion_errors[] = _tms__( '<strong>ERROR</strong>: Sorry, that email address is already used!', 'wordpress-tm-store' );
				}
			}
			# }}} validate usermail

			# {{{ validate username (called login in tms)
			if( $change_username == 1 )
			{
				$illegal_names = array(  'www', 'web', 'root', 'admin', 'main', 'invite', 'administrator' );

				$illegal_names = apply_filters( 'tms_new_users_gateway_alter_illegal_names', $illegal_names );

				if ( in_array( $requested_user_login, $illegal_names ) == true )
				{
					$profile_completion_errors[] = _tms__( '<strong>ERROR</strong>: That username is not allowed.', 'wordpress-tm-store' );
				}

				if ( strlen( $requested_user_login ) < 4 )
				{
					$profile_completion_errors[] = _tms__( '<strong>ERROR</strong>: Username must be at least 4 characters.', 'wordpress-tm-store' );
				}

				if ( strpos( ' ' . $requested_user_login, '_' ) != false )
				{
					// $profile_completion_errors[] = _tms__( '<strong>ERROR</strong>: Sorry, usernames may not contain the character &#8220;_&#8221;!', 'wordpress-tm-store' );
				}

				if ( preg_match( '/^[0-9]*$/', $requested_user_login ) )
				{
					$profile_completion_errors[] = _tms__( '<strong>ERROR</strong>: Sorry, usernames must have letters too!', 'wordpress-tm-store' );
				}

				if ( username_exists( $requested_user_login) )
				{
					$profile_completion_errors[] = _tms__( '<strong>ERROR</strong>: Sorry, that username already exists!', 'wordpress-tm-store' );
				}
			}
			# }}} validate username
			...
			$profile_completion_errors = apply_filters( 'tms_new_users_gateway_alter_profile_completion_errors', $profile_completion_errors );
	...
	if( $shall_pass == false )
	{
		$provider_name = tms_get_provider_name_by_id( $provider );
?>
<!DOCTYPE html>
	<head>
		...
		<div id="login">
			<div id="login-panel">
				...
				<div id="welcome">
					...
					<b><?php printf( _tms__( "Hi %s", 'wordpress-tm-store' ), htmlentities( $hybridauth_user_profile->displayName ) ); ?></b>
					<p><?php printf( _tms__( "You're now signed in with your %s account but you are still one step away of getting into our website", 'wordpress-tm-store' ), $provider ); ?>.</p>
				...
				<table id="mapping-options" border="0">
					<tr>
						<?php if( $linking_enabled == 1 ): ?>
							<td valign="top"  width="50%" style="text-align:center;">
								<h4><?php _tms_e( "Already have an account", 'wordpress-tm-store' ); ?>?</h4>
								<p style="font-size: 12px;"><?php printf( _tms__( "Link your existing account on our website to your %s ID.", 'wordpress-tm-store' ), $provider ); ?></p>
							</td>
						<?php endif; ?>

						<td valign="top"  width="50%" style="text-align:center;">
							<h4><?php _tms_e( "New to our website", 'wordpress-tm-store' ); ?>?</h4>
							<p style="font-size: 12px;"><?php printf( _tms__( "Create a new account and it will be associated with your %s ID.", 'wordpress-tm-store' ), $provider ); ?></p>
						</td>
					</tr>
					...
					<tr>
						<?php if( $linking_enabled == 1 ): ?>
							<td valign="top"  width="50%" style="text-align:center;">
								<input type="button" value="<?php _tms_e( "Link my account", 'wordpress-tm-store' ); ?>" class="button-primary" onclick="display_mapping_authenticate();" >
							</td>
						<?php endif; ?>

						<td valign="top"  width="50%" style="text-align:center;">
							<?php if( $require_email != 1 && $change_username != 1 && $extra_fields != 1 ): ?>
                                <input type="button" value="<?php _tms_e( "Create a new account", 'wordpress-tm-store' ); ?>" class="button-primary" onclick="document.getElementById('info-form').submit();" >
							<?php else : ?>
                                <input type="button" value="<?php _tms_e( "Create a new account", 'wordpress-tm-store' ); ?>" class="button-primary" onclick="display_mapping_complete_info();" >
                            <?php endif; ?>
						</td>
					</tr>
				</table>
				...
				<form method="post" action="<?php echo site_url( 'wp-login.php', 'login_post' ); ?>" id="link-form">
					<table id="mapping-authenticate" border="0">
						<tr>
							<td valign="top"  width="50%" style="text-align:center;">
								...
								<p><?php printf( _tms__( "Please enter your username and password of your existing account on our website. Once verified, it will linked to your % ID", 'wordpress-tm-store' ), ucfirst( $provider ) ) ; ?>.</p>
							</td>
						</tr>
						<tr>
							<td valign="bottom"  width="50%" style="text-align:left;">
								<label>
									<?php _tms_e( "Username", 'wordpress-tm-store' ); ?>
									<br />
									<input type="text" name="user_login" class="input" value=""  size="25" placeholder="" />
								</label>

								<label>
									<?php _tms_e( "Password", 'wordpress-tm-store' ); ?>
									<br />
									<input type="text" name="user_password" class="input" value="" size="25" placeholder="" />
								</label>

								<input type="submit" value="<?php _tms_e( "Continue", 'wordpress-tm-store' ); ?>" class="button-primary" >

								<a href="javascript:void(0);" onclick="display_mapping_options();" class="back-to-options"><?php _tms_e( "Back", 'wordpress-tm-store' ); ?></a>
							</td>
						</tr>
					</table>
					...
					<input type="hidden" id="action" name="action" value="wordpress_tm_account_linking">
					...
					<table id="mapping-complete-info" border="0">
						<tr>
							<td valign="top"  width="50%" style="text-align:center;">
								...
								<p><?php printf( _tms__( "Please fill in your information in the form below. Once completed, you will be able to automatically sign into our website through your %s ID", 'wordpress-tm-store' ), $provider_name ); ?>.</p>
							</td>
						</tr>
						<tr>
							<td valign="bottom"  width="50%" style="text-align:left;">
                                ...
                                <?php if( $require_email == 1 ): ?>
                                    <label>
                                        <?php _tms_e( "E-mail", 'wordpress-tm-store' ); ?>
                                        <br />
			...
			<p class="back-to-home">
				<a href="<?php echo home_url(); ?>">&#8592; <?php printf( _tms__( "Back to %s", 'wordpress-tm-store' ), get_bloginfo('name') ); ?></a>
			</p>
		</div>
