...
class RSSimpleProduct {
    ...
    public static function display_point_price_in_simple_product($price, $product) {
        ...
        if (get_option('rs_enable_disable_point_priceing') == '1') {
            ...
            if ($enabledpoints != '') {
                $point_price_info = "<span class='point_price_info'>$labelpoint$enabledpoints </span>";
                
                return $price." ".$point_price_info;                
            } else {
                return $price;
            }
        }else{
        ...
        if (is_shop() || is_product() || is_page() || is_product_category()) {
            if ($checkproducttype->is_type('simple') || ($checkproducttype->is_type('subscription'))) {
                ...
                if (get_option('rs_set_price_to_calculate_rewardpoints_by_percentage') == '1') {
                    ...
                    $getregularprice = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($post->ID, '_sale_price');
                    if($getregularprice==''){
                    $getregularprice = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($post->ID, '_regular_price');
                    }
                }
