                    ...
                    if ($current_date <= $expiry_date) {
                        if ($count_limit == '1') {
                            if (!in_array($current_userid, (array) $user_id)) {
                                ...
                                $updated_count = $current_usage_count + 1;
                                $get_point_for_url[$uniqueid]['current_usage_count'] = $updated_count;
                                $get_point_for_url[$uniqueid]['used_by'][] = $current_userid;
                            }
                        } else {
                            if ($count != '') {
    ...
    public static function rs_function_to_award_points_for_url_click() {
        if (isset($_GET['rsid'])) {
            ...
            if (is_user_logged_in()) {
                ...
                $current_usage_count = $get_point_for_url[$uniqueid]['current_usage_count'];                
                ...
                <div class="rs_success_msg_for_pointurl"><?php if ($current_usage_count <= $count_limit) {echo $replace_name;}else{echo $replace_name;} ?></div>
                <?php
            }
        }
    }
