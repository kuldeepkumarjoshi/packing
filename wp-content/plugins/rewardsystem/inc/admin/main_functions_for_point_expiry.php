...
class RSPointExpiry {
    ...
    public static function update_revised_points_for_user($order_id) {
        ...
        foreach ($order->get_items() as $item) {
            ...
            if (array_key_exists($variationid, $product_ids)) {
                self::rs_insert_the_selected_level_revised_reward_points($redeempoints, $checked_level_for_reward_points, $variationid, $variationid, $itemquantity, $orderuserid, $termid, $equearnamt, $equredeemamt, $order_id, $product_ids);
            }
            $gettotalearnedpoints = $wpdb->get_results("SELECT SUM((earnedpoints)) as availablepoints FROM $table_name WHERE orderid = $order_id", ARRAY_A);
            $totalearnedpoints = ($gettotalearnedpoints[0]['availablepoints'] != NULL) ? $gettotalearnedpoints[0]['availablepoints'] : 0;
            $totalredeempoints = $redeempoints;
    ...
    public static function rs_insert_the_selected_level_revised_reward_points($pointsredeemed, $level, $productid, $variationid, $itemquantity, $orderuserid, $termid, $equearnamt, $equredeemamt, $order_id, $product_ids) {
        ...
        if (get_option('rs_set_price_to_calculate_rewardpoints_by_percentage') == '1') {
            ...
            $getregularprice1 = RSFunctionForSavingMetaValues::rewardsystem_get_post_meta($productid, '_sale_price');
            if ($getregularprice1 == '') {
                ...
                if ($order->status == 'completed') {
                    ...
                    foreach ($order->get_items() as $item) {
                        if (get_option('rs_set_price_to_calculate_rewardpoints_by_percentage') == '1') {
                            ...
                            $getregularprice = get_post_meta($item['product_id'], '_sale_price', true);
                            if ($getregularprice == '') {
                                $getregularprice = get_post_meta($item['product_id'], '_regular_price', true);
                            }
                        }
        ...
        switch ($checkpoints) {
            ...
            case 'IMPOVR':
                if ($masterlog == true) {
                    ...
                    $replaceproductid2 = str_replace('[name]', $earnpoints, $getmsgrpg);
                    return $replaceproductid2;
                    break;
                } else {
                    $getmsgrpg = get_option('_rs_localize_log_for_import_override');
